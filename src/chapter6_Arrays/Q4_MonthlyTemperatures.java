...
public class Q4_MonthlyTemperatures {
		...
		do
		{
			...
			switch (choice)
			{
				...
				case '4': 	displayDays(temperature);
							break;
				case '5': 	double maxTemp = findMax(temperature);
	...
	// method that returns max temp recorded
	static double findMax(double[][] temperatureIn)
	{
		...
		for (int week = 1; week <= temperatureIn.length; week++)
		{
			...
			for (int day = 1; day <= temperatureIn[0].length; day++) 
			{
				if (temperatureIn[week-1][day-1] > result) result = temperatureIn[week-1][day-1];
			}
		}
		return result;
	}
